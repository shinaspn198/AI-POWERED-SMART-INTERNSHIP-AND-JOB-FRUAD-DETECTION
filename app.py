...
# ======================================================
# MODEL LOAD — cached so loads only once per session
# ======================================================
...
def get_strong_fraud_reasons(text):
    ...
    reasons = [kw for kw in FRAUD_KEYWORDS if kw.lower() in text_lower]
    patterns = {
        "payment request":       r"(pay|payment|fee|deposit|amount|transfer|charges)\s*(₹|\$|inr|usd)?\s*\d+",
        ...
        "urgent pressure":       r"urgent|immediately|limited\s+seats|apply\s+immediately",
...
with tab1:
    ...
    if analyze:
        if not final_text:
            ...
            with st.spinner("🧠 Generating LIME explanation (faster now)..."):
                exp = explainer.explain_instance(
                    final_text, predict_proba_fn,
                    num_features=12,
                    num_samples=500   # 10x faster than default 5000
                )
